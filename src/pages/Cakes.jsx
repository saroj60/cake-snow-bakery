...
const Cakes = () => {
  ...
  const handleAddToCartClick = (cake) => {
    addToCart({
      ...cake,
      id: `${cake.id}-1`,
      price: typeof cake.price === 'string' ? parseFloat(cake.price.replace(/,/g, '')) : cake.price,
      messageOnCake: '',
      flavor: 'Default',
      shape: 'Round'
    });
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          <h1 className="font-headline-xl text-3xl md:text-5xl text-[#0D47A1] font-bold mb-4">
            {searchQuery ? `Search Results for "${searchQuery}"` : "Our Delicious Cakes"}
          </h1>
          <p className="text-[#6B1FA6] max-w-2xl mx-auto">Explore our wide variety of premium cakes perfect for any celebration.</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {filteredCakes.map((cake) => (
            <div key={cake.id} className="bg-white dark:bg-[#1D0A2D] rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all border border-[#0D47A1]/10 group flex flex-col">
              <Link 
                className="aspect-square overflow-hidden rounded-xl mb-4 bg-[#FDFBF7] dark:bg-[#12041C] cursor-pointer block relative"
                to={`/product/${cake.id}`}
              >
                <img 
                ...
                />
                {cake.tags && cake.tags[0] && (
                  <span className="absolute top-2 left-2 bg-[#F5C242] text-[#0D47A1] text-xs font-bold px-2 py-1 rounded-full shadow-sm">
                    {cake.tags[0]}
                  </span>
                )}
              </Link>
              <div className="flex flex-col items-start mb-2 min-h-[44px]">
                <Link 
                  className="font-bold text-lg text-[#0D47A1] line-clamp-2 cursor-pointer hover:text-[#6B1FA6] transition-colors leading-tight"
                  to={`/product/${cake.id}`}
                >
                  {cake.name}
                </Link>
              ...
              <div className="flex items-center justify-between mb-4 mt-auto">
                <span className="font-bold text-lg text-[#6B1FA6] whitespace-nowrap">Rs. {cake.price}</span>
                <div className="flex items-center gap-1 text-[#F5C242] text-sm">
                  <Star size={14} fill="currentColor" />
                  <span className="text-[#0D47A1] font-medium">{cake.rating || '4.8'}</span>
                </div>
              </div>
              <button 
          ...
          {filteredCakes.length === 0 && !isLoading && (
            <div className="col-span-full text-center py-12">
              <p className="text-[#6B1FA6] text-xl font-medium">No cakes found matching your criteria.</p>
              <button 
                onClick={() => { setActiveFilter('All'); setPriceRange(5000); }}
          ...
          {isLoading && (
            <div className="col-span-full text-center py-12">
              <p className="text-[#0D47A1] text-xl font-medium animate-pulse">Loading delicious cakes...</p>
            </div>
          )}
        </div>
      </section>
